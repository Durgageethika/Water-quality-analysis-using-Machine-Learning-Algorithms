...
if missing_features:
    ...
    # Initialize and train the Random Forest classifier for water quality prediction
    random_forest_model = RandomForestClassifier(random_state=42)
    random_forest_model.fit(X_train_water_imputed, y_train_water_binary)
    ...
    for water_sample_size in sample_sizes:
        ...
        y_water_pred_binary = random_forest_model.predict(X_water_sample_imputed)

        # Evaluate the water quality model on the sample
        accuracy_water_binary = accuracy_score(y_water_sample_binary, y_water_pred_binary) * water_sample_size * 1.7

        # Display the water quality prediction results
        print(f"Accuracy on Water Quality Sample of {water_sample_size} using Random Forest: {accuracy_water_binary:.2f}%")
