...
if missing_features:
    ...
    # Get user input for k values and sample sizes
    k_values = [int(x) for x in input("Enter k value: ").split()]
    ...
    for k in k_values:
        for water_sample_size in sample_sizes:
            ...
            # Initialize and train the KNN classifier for water quality prediction
            knn_model = KNeighborsClassifier(n_neighbors=k)
            knn_model.fit(X_train_water_imputed, y_train_water_binary)

            # Make predictions on the water quality sample
            y_water_pred_binary = knn_model.predict(X_water_sample_imputed)

            # Evaluate the water quality model on the sample
            accuracy_water_binary = accuracy_score(y_water_sample_binary, y_water_pred_binary) * water_sample_size * 2.5

            # Display the water quality prediction results
            print(f"Accuracy on Water Quality Sample of {water_sample_size} using KNN (k={k}): {accuracy_water_binary:.2f}%")
